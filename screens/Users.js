...
import {URL, getAllUser} from '../const';
...
const Users = (props) => {
  ...
  useEffect(() => {
    getAllUserFunction();
    // let response = await axios.post(`${URL}${getAllUser}`);
    // console.log("response is ==>",response)
  }, []);

  const getAllUserFunction = async () => {
    // console.log(`${URL}${getAllUser}`);
    let response = await axios.get(`${URL}${getAllUser}`);
  ...
  const clickEventListener = (item) => {
    ...
    props.navigation.navigate('Explorer',{receiver:item});
